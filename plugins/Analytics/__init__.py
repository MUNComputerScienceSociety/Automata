...
class Analytics(AutomataPlugin):
    ...
    async def on_ready(self):
        ...
        for channel in await cursor.to_list(length=None):
            guild_channel = self.bot.get_guild(514110851016556567).get_channel(channel["channel_id"])
    ...
    async def on_message(self, message: discord.Message):
        ...
        if not isinstance(message.channel, discord.abc.PrivateChannel):
            existing_count = await self.message_counts.find_one({"channel_id": message.channel.id})
            if existing_count is not None:
                await self.message_counts.update_one({"channel_id": message.channel.id}, {"$inc": {"count": 1}})
            else:
                await self.message_counts.insert_one({"channel_id": message.channel.id, "count": 1})
